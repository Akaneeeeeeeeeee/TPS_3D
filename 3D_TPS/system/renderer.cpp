 ...
 * @param count スポットライトの数
 */
void Renderer::SetSpotLights(const SpotLightGPU* lights, int count)
{
	if (!m_DeviceContext) return;

	count = std::clamp(count, 0, MAX_SPOT_LIGHT);
	m_SpotLightCB.Count = count;

	// いったんゼロで埋める（未使用ライトのゴミ回避）
	for (int i = 0; i < MAX_SPOT_LIGHT; ++i)
	{
		m_SpotLightCB.Lights[i] = SpotLightGPU{};
		m_SpotLightCB.Lights[i].Params2 = Vector4(0, 0, 0, 0); // enabled=0
	}

	// 有効分だけコピー
	for (int i = 0; i < count; ++i)
	{
		m_SpotLightCB.Lights[i] = lights[i];
		// enabled を保証
		m_SpotLightCB.Lights[i].Params2.x = 1.0f;
	}

	m_DeviceContext->UpdateSubresource(m_SpotLightBuffer.Get(), 0, nullptr, &m_SpotLightCB, 0, 0);

	// 念のためバインド（描画途中で上書きされる設計なら毎回呼ぶ）
	m_DeviceContext->VSSetConstantBuffers(6, 1, m_SpotLightBuffer.GetAddressOf());
	m_DeviceContext->PSSetConstantBuffers(6, 1, m_SpotLightBuffer.GetAddressOf());
}
