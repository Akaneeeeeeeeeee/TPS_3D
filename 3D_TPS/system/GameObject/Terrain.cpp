...
float Terrain::GetHeight2(Vector3 pos)
{
	std::array<Terrain::Face, 2> retfaces;			// 矩形を構成する三角形構成頂点インデックスを２個分
...
// 平面の方程式を生成する
void Terrain::MakePlaneEquatation() {
...
// 何番目の四角形かを見つける
int Terrain::GetSquareNo(Vector3 pos)
{
	// 範囲チェック
...
bool Terrain::GetFace(Vector3 pos, std::array<Terrain::Face, 2>& retdata) {
...
void Terrain::GetFaceVertex(int triangleindex, std::array<Vector3, 3>& vertices) {
