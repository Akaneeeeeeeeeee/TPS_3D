...
// ==============================
#if defined(_DEBUG)
inline void AssimpDebugLog(const char* fmt, ...)
{
	va_list args;
	va_start(args, fmt);
...
#define ASSIMP_LOG(...) AssimpDebugLog(__VA_ARGS__)
#else
#define ASSIMP_LOG(...) ((void)0)
#endif
...
{
	ASSIMP_LOG("[Scene]");
	ASSIMP_LOG("  meshes     : %u", s->mNumMeshes);
	ASSIMP_LOG("  materials  : %u", s->mNumMaterials);
	ASSIMP_LOG("  textures(embedded): %u", s->mNumTextures);
	ASSIMP_LOG("  animations : %u", s->mNumAnimations);
	ASSIMP_LOG("  cameras    : %u", s->mNumCameras);
	ASSIMP_LOG("  lights     : %u", s->mNumLights);
}

static void DumpTex(aiMaterial* mat, aiTextureType type, const char* label)
{
	unsigned n = mat->GetTextureCount(type);
	ASSIMP_LOG("  %s = %u", label, n);
	for (unsigned i = 0; i < n; ++i)
	{
		aiString p;
		if (mat->GetTexture(type, i, &p) == AI_SUCCESS)
			ASSIMP_LOG("    [%u] %s", i, p.C_Str());
	}
}
