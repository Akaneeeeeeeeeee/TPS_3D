...
HRESULT IShader::Analyze_to_Adjust(void* pData, UINT size)
{
    ...
    D3DReflect(pData, size, IID_PPV_ARGS(&reflection));

    D3D11_SHADER_DESC shaderDesc;
    reflection->GetDesc(&shaderDesc);

    ShaderReflection shaderRef;
    ...
    for (UINT i = 0; i < shaderDesc.ConstantBuffers; ++i)
    {
        ...
        D3D11_SHADER_INPUT_BIND_DESC bindDesc;
        reflection->GetResourceBindingDescByName(bufDesc.Name, &bindDesc);

        shaderRef.cbuffers.push_back({ bufDesc.Name, bindDesc.BindPoint, bufDesc.Size });
    }
    ...
    for (UINT i = 0; i < shaderDesc.BoundResources; ++i)
    {
        ...
        {
            shaderRef.srvs.push_back({ bindDesc.Name, bindDesc.BindPoint });
        }
    }

    m_ShaderReflection = shaderRef;

    // 実際のVS/PS作成はCreate()で行う
    return Create(pData, size);
}
