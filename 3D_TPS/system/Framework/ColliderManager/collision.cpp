	...
	namespace GE {
		...
		namespace Collision {
				...
				const Vector3& vecDistance)		// 中心座標を結んだベクトル
			{
				...
				// 分離軸に射影した中心間を結ぶベクトル
				_distance = vecDistance.Dot(vecSeparate);	// 中心座標を結んだベクトルと分離軸の内積を求める
				_distance = fabsf(_distance);		// 絶対値を求める

				// 分離軸上にボックスAを射影した影の長さ
				float _shadow_A_Length = 0;

				// 分離軸上にボックスBを射影した影の長さ
				float _shadow_B_Length = 0;

				// ボックスAの影を算出
				Vector3 _ShadowA = {};

				// X軸を分離軸に射影
				_ShadowA.x = vecSeparate.Dot(ObbA.axisX);	// 分離軸とX軸の内積を求める
				_ShadowA.x = fabsf(_ShadowA.x * ObbA.lengthx / 2.0f);	// 絶対値を求める

				// Y軸を分離軸に射影
				_ShadowA.y = vecSeparate.Dot(ObbA.axisY);	// 分離軸とY軸の内積を求める
				_ShadowA.y = fabsf(_ShadowA.y * ObbA.lengthy / 2.0f);	// 絶対値を求める

				// Z軸を分離軸に射影
				_ShadowA.z = vecSeparate.Dot(ObbA.axisZ);	// 分離軸とZ軸の内積を求める
				_ShadowA.z = fabsf(_ShadowA.z * ObbA.lengthz / 2.0f);	// 絶対値を求める

				// 分離軸に射影した影の長さを求める
				_shadow_A_Length = _ShadowA.x + _ShadowA.y + _ShadowA.z;	// 影の長さを求める

				// ボックスBの影を算出
				Vector3 _ShadowB = {};

				// X軸を分離軸に射影
				_ShadowB.x = vecSeparate.Dot(ObbB.axisX);	// 分離軸とX軸の内積を求める
				_ShadowB.x = fabsf(_ShadowB.x * ObbB.lengthx / 2.0f);	// 絶対値を求める

				// Y軸を分離軸に射影
				_ShadowB.y = vecSeparate.Dot(ObbB.axisY);	// 分離軸とY軸の内積を求める
				_ShadowB.y = fabsf(_ShadowB.y * ObbB.lengthy / 2.0f);	// 絶対値を求める

				// Z軸を分離軸に射影
				_ShadowB.z = vecSeparate.Dot(ObbB.axisZ);	// 分離軸とZ軸の内積を求める
				_ShadowB.z = fabsf(_ShadowB.z * ObbB.lengthz / 2.0f);	// 絶対値を求める

				// 分離軸に射影した影の長さを求める
				_shadow_B_Length = _ShadowB.x + _ShadowB.y + _ShadowB.z;	// 影の長さを求める

				if (_distance > _shadow_A_Length + _shadow_B_Length) {
					// 分離軸に射影した影の長さの和よりも中心間の距離が大きい場合は衝突していない
