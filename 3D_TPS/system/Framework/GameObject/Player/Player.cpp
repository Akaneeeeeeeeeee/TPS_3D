...
void Player::Start(void)
{
    ...
    if (m_pCamera)
    {
        m_CamAzimuth = m_pCamera->GetAzimuth();
        m_CamElevation = m_pCamera->GetElevation();

        m_CamRadiusCur = m_pCamera->GetRadius();
        m_CamShoulderCur = 0.0f;
        m_CamLookAtHeightCur = CAMERA_LOOK_AT_HEIGHT;
        m_CamNearCur = m_pCamera->GetNearPlane();

        m_pCamera->SetShoulderOffset(0.0f);
    }
...
void Player::ApplyStance(const InputState& in)
{
    ...
    if (m_pCharaVirtualComp)
    {
        m_pCharaVirtualComp->SetStance(
            in.isCrouching ? CharacterVirtualComponent::Stance::Crouch
...
void Player::ApplyMoveToCharacterVirtual(const Vector3& moveDir, float moveAmount, bool wantsJump)
{
    ...
    if (m_pCharaVirtualComp)
    {
        m_pCharaVirtualComp->SetMoveInput(moveDir, moveAmount);
...
void Player::UpdateMovementAnimation(const InputState& in)
{
    ...
    {
        m_pAnimComp->Play(in.isCrouching ? AnimType::Crouch : AnimType::Idle, ANIM_BLEND_TIME_SEC);
        m_pAnimComp->SetPlaybackSpeed(ANIM_SPEED_IDLE);
    }
    else
    {
